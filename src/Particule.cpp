...
void Particule::update(Parameters p) {
    ...
    //updating event
    if (_noiseEventGo && p.P_ACTIVE_NOISE_E) noiseEvent(p);

    if(p.FRAMECOUNT % p.P_TIME_TIME_E == 0 && p.FRAMECOUNT != 0) { 
        if(p.P_ACTIVE_ANGLE_E) timedEvent(p);
        _noiseEventGo = !_noiseEventGo;
    }

    if(probabilityPercent(p.P_TIME_LUCK_E) && p.P_ACTIVE_ANGLE_E) timedEvent(p);

    if (p.FRAMECOUNT % p.P_SIZE_TIME_E == 0) {
        if (probabilityPercent(p.P_SIZE_LUCK_E)) {
            if(p.P_ACTIVE_SIZE_E) sizeEvent(p);
            _sizeEventGo = !_sizeEventGo;
        }
    }

    if(p.P_ACTIVE_MARKOV_E) markovEvent(p.P_MARKOV_COEF1_E, p.P_MARKOV_COEF2_E);
...
void Particule::displayParticule(p6::Context &ctx, Parameters p) {
    ...
    if(!p.P_SHAPE) ctx.square(p6::Center(_pos), _size + p.P_SIZE_ADD);
    else ctx.circle(p6::Center(_pos), _size + p.P_SIZE_ADD);
    ...
    if(!p.P_SHAPE) ctx.square(p6::Center(_pos[0] + 2 * _size + p.P_SIZE_ADD, _pos[1]), _size + p.P_SIZE_ADD);
    else ctx.circle(p6::Center(_pos[0] + 2 * _size + p.P_SIZE_ADD, _pos[1]), _size + p.P_SIZE_ADD);
